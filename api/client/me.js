  ...
  await requireAuth(req, res, async () => {
    ...
    if (profile.role === 'client') {
      const { data: insc } = await supabaseAdmin
        .from('inscripciones')
        .select('*, courses:course_id (id, nombre, tipo, horas, duracion_dias)')
        .eq('client_id', userId);
      ...
      if (insc) {
        inscripciones = insc.map(i => {
          ...
          const diasRestantes = i.vencimiento
            ? Math.max(0, Math.ceil((new Date(i.vencimiento).getTime() - Date.now()) / DAY_MS))
            : null;
          return {
            ...
            horasRestantes: course.tipo === 'servicio' && i.horas_asignadas
              ? i.horas_asignadas - i.horas_usadas : null
          };
        });
      }
    }
